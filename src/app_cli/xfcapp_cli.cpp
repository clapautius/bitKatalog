...
int printToStdout(unsigned int lDepth, const std::string &lPath, Xfc &, xmlNodePtr lpNode,
                  void *lpParam);
int findInTree(unsigned int lDepth, const std::string &lPath, Xfc &, xmlNodePtr lpNode,
               void *lpParam);
void initStuff();
...
{
    cout << "Usage: bitKatalog_cli [--verbose-level <level (0-3)>]  "
            "[--xfclib-verbose-level <level (0-3)>]";
    cout << endl
         << endl
         << XFCAPP_NAME << ", version: " << XFCAPP_VERSION << endl
         << endl;
}
            ...
            case 0:
                switch (option_index) {
                    ...
                    case 1:  // --verbose-level
                        ...
                        gcLog << xfcInfo << "verbose level is " << gcLog.getVerboseLevel()
                              << eol;
                        break;

                    case 2:  // --xfclib-verbose-level
                        gLog.setVerboseLevel((unsigned int)atoi(optarg));
                        gcLog << xfcInfo << "verbose level for xfclib is "
                              << gLog.getVerboseLevel() << eol;
                        break;
                }
                break;
...
static int processCommand(std::vector<std::string> &rCmd)
{
    ...
    if (rCmd[0].empty())
        ...
        displayMessage(
            "    if path is a dir and ends with '/', the root dir is not added to xml "
            "tree");
            ...
            if (!lError) {
                try {
                    ...
                    if (rCmd[0] == "add" || rCmd[0] == "add-allSums") {
                        ...
                        gXfc.addPathToXmlTree(rCmd[1], lMaxDepth, NULL, cbList1, cbList2,
                                              rCmd[2]);
                    } else
                        gXfc.addPathToXmlTree(rCmd[1], lMaxDepth, NULL, cbList1, cbList2,
                                              rCmd[2]);
        ...
        if (gXfc.getState() == 0) {
            ...
            displayError(
                "There is a catalog in memory. Use force_new to create a new one");
        }
            ...
            if (rCmd[1] == "disks") {
                try {
                    ...
                    for (unsigned short i = 0; i < lVect.size(); i++) {
                        ...
                        lDescription =
                            gXfc.getDescriptionOfNode(gXfc.getNodeForPath(lVect[i]));
...
static void displayDiff(string left, string middle, string right)
{
    displayMessage(" -* ", left);
            ...
            switch (differences[i].type) {
                ...
                case eDiffSize:
                    str1 = differences[i].name +
                           " (catalog size: " + differences[i].catalogValue + ")";
                    str2 = differences[i].name +
                           " (disk size: " + differences[i].diskValue + ")";
                ...
                case eDiffSha256Sum:
                    str1 = differences[i].name +
                           " (catalog sha256: " + differences[i].catalogValue + ")";
                    str2 = differences[i].name +
                           " (disk sha256: " + differences[i].diskValue + ")";
                    displayDiff(str1, " ! ", str2);
                    str1.clear();
                    str2.clear();
                    break;
                case eDiffSha1Sum:
                    str1 = differences[i].name +
                           " (catalog sha1: " + differences[i].catalogValue + ")";
                    str2 = differences[i].name +
                           " (disk sha1: " + differences[i].diskValue + ")";
                    displayDiff(str1, " ! ", str2);
                    str1.clear();
                    str2.clear();
                    break;
                case eDiffErrorOnDisk:
                    str1 = differences[i].name;
                    str2 = differences[i].name + " - error reading details from disk: " +
...
int printToStdout(unsigned int lDepth, const string &lPath, Xfc &lrXfc, xmlNodePtr lpNode,
                  void *lpParam)
{
    for (unsigned short i = 0; i < lDepth; i++) cout << " ";
    cout << (const char *)lpNode->name << ":" << lrXfc.getNameOfElement(lpNode).c_str()
         << endl;
    return 0;
}
