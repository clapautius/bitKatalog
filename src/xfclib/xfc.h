...
{
   public:
    typedef enum { eUnknown = 0, eDisk, eFile, eDir, eRoot } ElementType;

    typedef int (*XmlParamForFileCallback)(std::string fileName, std::string *pParam,
                                           std::string *pValue,
                                           volatile const bool *pAbortFlag);

    typedef int (*XmlParamForFileChunkCallback)(const char *buf, uint len,
                                                bool isFirstChunk, bool isLastChunk,
    ...
    void parseFileTree(ParserFuncType callBackFunc,
                       void *pParam = NULL) throw(std::string);

    void parseDisk(ParserFuncType callBackFunc, std::string diskName,
                   void *pParam = NULL);
    ...
    std::string getDescriptionOfNode(xmlNodePtr pNode,
                                     xmlNodePtr *pDescriptionNode = NULL) const
        throw(std::string);

    // maxDepth - -1
    void addPathToXmlTree(std::string lPath, int lMaxDepth,
                          volatile const bool *pAbortFlag,
                          std::vector<XmlParamForFileCallback>,
                          std::vector<XmlParamForFileChunkCallback>, std::string diskId,
                          std::string diskCat = "", std::string diskDescr = "",
                          std::string diskLabel = "") throw(std::string);
    ...
    std::map<std::string, std::string> getDetailsForNode(xmlNodePtr lpNode) throw(
        std::string);

    std::vector<std::string> getLabelsForNode(xmlNodePtr lpNode) throw(std::string);

    xmlNodePtr addNewDiskToXmlTree(std::string diskName, std::string diskCategory = "",
                                   std::string diskDescription = "",
                                   std::string diskLabel = "",
                                   std::string diskCDate = "") throw(std::string);
    ...
    void addLabelsTo(xmlNodePtr node, const std::vector<std::string> *pLabels,
                     bool checkDuplicate = false) throw(std::string);

    void removeLabelFrom(std::string path, std::string label) throw(std::string);

    void addLabelRecTo(std::string path, std::string label) throw(std::string);

    void addLabelsRecTo(std::string path,
                        std::vector<std::string> labels) throw(std::string);

    void removeLabelRecFrom(std::string path, std::string label) throw(std::string);

    void setDescriptionOf(std::string path, std::string description) throw(std::string);

    std::string getCDate(std::string diskName, xmlNodePtr *pCDateNode = NULL) const
        throw(std::string);

    void setCDate(std::string diskName, std::string cDate) throw(std::string);

    // ret "" if no shasum
    std::string getChecksumOf(std::string path, std::string sumType = "") const
        throw(std::string);

    // ret "" if no shasum
    std::string getChecksumOf(xmlNodePtr pNode, std::string sumType = "") const
        throw(std::string);
    ...
    int verifyDirectory(std::string catalogPath, std::string diskPath, uint pathPrefixLen,
                        std::vector<EntityDiff> *pDiffs,
   ...
   private:
    ...
    // lPath - fs path
    xmlNodePtr addDirToXmlTree(xmlNodePtr pParent, std::string path) throw(std::string);

    void parseRec(uint depth, const std::string &path, xmlNodePtr pNode,
                  ParserFuncType func, void *pParam) throw(std::string);

    /// root level is 0
    /// maxDepth = -1 -> ignore max depth
    void recAddPathToXmlTree(xmlNodePtr pCurrentNode, std::string path, int level,
                             int maxDepth, volatile const bool *pAbortFlag,
                             std::vector<XmlParamForFileCallback>,
                             std::vector<XmlParamForFileChunkCallback>,
                             bool lSkipFirstLevel = false) throw(std::string);

    xmlNodePtr getNodeForPathRec(std::string path, xmlNodePtr pNode) const throw();
    ...
    std::string getCDate(xmlNodePtr pDiskNode, xmlNodePtr *pCDateNode = NULL) const
        throw(std::string);

    EntityDiff compareItems(std::string catalogName, XfcEntity &rEnt,
                            std::string diskPath, uint pathPrefixLen, bool &rShaMissing,
                            volatile const bool *pAbortFlag = NULL);
