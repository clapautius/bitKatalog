 ...
 *
 **/
bool waitChildProcess(pid_t process, volatile const bool *pAbortFlag, int &status,
                      int inputFd, string &cmdOutput)
{
    int rc, len;
    char buf[100];
    bool killed = false;
    ...
    while (true) {
        ...
        if (process == rc) {  // finished
            gLog << xfcDebug << __FUNCTION__ << ": waitpid returned " << rc
                 << ". status=" << status << eol;
            ...
            if (pAbortFlag && *pAbortFlag) {
                ...
                cout.flush();
                if (!killed) {
                    gLog << xfcInfo << __FUNCTION__
                         << ": Abort flag is on, killing process, pid=" << process << eol;
                    cout.flush();
                    kill(process, 15);
                    killed = true;
                }
...
std::string executeCommand(const char *lpPrg, volatile const bool *pAbortFlag,
                           const char *lpArg1, const char *lpArg2, const char *lpArg3,
                           const char *lpArg4) throw(std::string)
{
    pid_t prPid;
    int fds[2];
    std::string output;
 ...
 **/
int saveWithBackup(Xfc *pXfcCat, std::string path, std::string &error)
{
    int rc = 0;
    string name;
...
string vectorWStringsToString(vector<string> &vect, const char *pDelim)
{
    string str;
...
bool contains(vector<string> vect, string elt)
{
    for (uint i = 0; i < vect.size(); i++)
        if (vect[i] == elt)
            return true;
...
long int stringToInt(std::string str) throw(std::string)
{
    long int sizeNum;
    char *pRet = NULL;
