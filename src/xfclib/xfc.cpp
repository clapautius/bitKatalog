 ...
 * @retval -2 : error;
 **/
int addLabelCallback(unsigned int depth __attribute__((unused)), const string &path,
 ...
 * @retval -2 : error;
 **/
int addLabelsCallback(unsigned int depth __attribute__((unused)), const string &path,
                      Xfc &rXfc, xmlNodePtr pNode, void *pParam)
{
...
void Xfc::saveToFile(std::string lPath, int lParams) throw(std::string)
{
    if (xmlSaveFormatFileEnc(lPath.c_str(), getXmlDocPtr(), "utf-8", lParams) == -1) {
 ...
 **/
void Xfc::parseFileTree(ParserFuncType callback_func, void *p_param) throw(std::string)
{
    XFC_TIMING_FN_ENTRY;
    // :fixme: - check state
    xmlNodePtr p_child;
...
void Xfc::parseRec(unsigned int depth, const std::string &path, xmlNodePtr p_node,
                   ParserFuncType callback_func, void *p_param) throw(std::string)
{
    int rc;
    xmlNodePtr p_child;
    std::string s;
 ...
 **/
bool Xfc::isDirDiskFileOrRoot(xmlNodePtr pNode) const throw()
{
...
std::string Xfc::getNameOfFile(xmlNodePtr lpNode) const throw(std::string)
{
...
std::string Xfc::getNameOfDisk(xmlNodePtr lpNode) const throw(std::string)
{
    if (isDisk(lpNode))
        return getNameOfElement(lpNode);
    else
...
std::string Xfc::getNameOfElement(xmlNodePtr p_node) const throw(std::string)
{
// :fixme: - check state
#if defined(XFC_DEBUG)
...
void Xfc::setNameOfElement(xmlNodePtr pNode, std::string newName) throw(std::string)
{
...
Xfc::ElementType Xfc::getTypeOfElement(xmlNodePtr p_node) const throw(std::string)
{
    // :fixme: - check state
...
std::string Xfc::getDescriptionOfNode(xmlNodePtr lpNode,
                                      xmlNodePtr *lpDescriptionNode) const
    throw(std::string)
...
void Xfc::addPathToXmlTree(string lPath, int lMaxDepth, volatile const bool *pAbortFlag,
                           vector<XmlParamForFileCallback> cbList1,
                           vector<XmlParamForFileChunkCallback> cbList2, string lDiskId,
                           string lDiskCategory, string lDiskDescription,
                           string lDiskLabel) throw(std::string)
{
...
xmlNodePtr Xfc::addNewDiskToXmlTree(std::string lDiskName, std::string lDiskCategory,
                                    ...
                                    std::string lDiskCDate) throw(std::string)
// :fixme: - check for duplicates
{
    ...
    lpNode =
        xmlNewTextChild(lpRoot, NULL, (xmlChar *)"disk", NULL);  // :bug: - conversion
    if (lpNode == NULL)
        throw std::string("Xfc::addNewDiskToXmlTree(): xmlNewTextChild() error");

    if (xmlNewTextChild(lpNode, NULL, (xmlChar *)"name", (xmlChar *)lDiskName.c_str()) ==
        NULL) {
        throw string(__FUNCTION__) + "xmlNewTextChild() error";
    }

    if (lDiskCategory != "")
        xmlNewTextChild(lpNode, NULL, (xmlChar *)"category",
                        (xmlChar *)lDiskCategory.c_str());

    if (lDiskDescription != "")
        xmlNewTextChild(lpNode, NULL, (xmlChar *)"description",
                        (xmlChar *)lDiskDescription.c_str());
...
void Xfc::deleteDiskFromXmlTree(std::string diskName) throw(std::string)
{
 ...
 * helper function
 **/
map<string, string> Xfc::bufferCallbacks(string path,
                                         vector<XmlParamForFileChunkCallback> &cbList2,
...
/**
 **/
xmlNodePtr Xfc::addFileToXmlTree(xmlNodePtr lpNode, string path,
                                 vector<XmlParamForFileCallback> cbList1,
                                 vector<XmlParamForFileChunkCallback> cbList2,
                                 volatile const bool *pAbortFlag) throw(std::string)
{
    ...
    // name
    xmlNewTextChild(lpNewNode, NULL, (xmlChar *)"name",
                    (xmlChar *)lName.c_str());  // :bug:

    // size
    FileSizeType lSize = getFileSize(path);
    std::ostringstream lStrOut;
    lStrOut << lSize;
    xmlNewTextChild(lpNewNode, NULL, (xmlChar *)"size",
                    (xmlChar *)lStrOut.str().c_str());  // :bug:
...
xmlNodePtr Xfc::addDirToXmlTree(xmlNodePtr lpNode, std::string lPath) throw(std::string)
{
...
void Xfc::recAddPathToXmlTree(xmlNodePtr lpCurrentNode, std::string lPath, int lLevel,
                              int lMaxDepth, volatile const bool *pAbortFlag,
                              std::vector<XmlParamForFileCallback> cbList1,
                              std::vector<XmlParamForFileChunkCallback> cbList2,
                              bool lSkipFirstLevel) throw(std::string)
{
...
xmlNodePtr Xfc::getNodeForPath(std::string lPath) const throw()
{
    xmlNodePtr lpRoot;
...
xmlNodePtr Xfc::getNodeForPathRec(std::string lPath, xmlNodePtr lpNode) const throw()
{
    xmlNodePtr lNameNode;
 ...
 * @sa getLabelsForNode
 **/
std::map<std::string, std::string> Xfc::getDetailsForNode(xmlNodePtr lpNode) throw(
    std::string)
{
...
vector<string> Xfc::getLabelsForNode(xmlNodePtr lpNode) throw(string)
{
...
int Xfc::getState() const throw() { return mState; }

string Xfc::getParamValueForNode(xmlNodePtr pNode, string param)
{
    string ret;
    xmlNodePtr pChildNode;
 ...
 **/
void Xfc::addLabelTo(std::string lPath, std::string lLabel) throw(std::string)
{
    xmlNodePtr lpNode;
    ...
    lpNewNode = xmlNewTextChild(lpNode, NULL, (xmlChar *)"label",
                                (xmlChar *)lLabel.c_str());  // :bug:
    ...
    if (lpInsertBeforeNode != NULL) {
        if (xmlAddPrevSibling(lpInsertBeforeNode, lpNewNode) == NULL)
            throw std::string(
                "Xfc::addLabelToNode() error - error in xmlAddPrevSibling()");
    }
}
 ...
 **/
void Xfc::addLabelsTo(xmlNodePtr pNode, const vector<string> *pLabels,
                      bool checkDuplicate) throw(std::string)
{
    ...
    for (uint i = 0; i < pLabels->size(); i++) {
        ...
        if (pInsertBeforeNode != NULL) {
            if (xmlAddPrevSibling(pInsertBeforeNode, pNewNode) == NULL)
                throw std::string(__FUNCTION__) + " - error in xmlAddPrevSibling()";
        }
    }
}

void Xfc::removeLabelFrom(std::string lPath, std::string lLabel) throw(std::string)
{
    xmlNodePtr lpNode;
    lpNode = getNodeForPath(lPath);
    if (lpNode == NULL)
 ...
 **/
void Xfc::addLabelRecTo(string path, string label) throw(string)
{
    xmlNodePtr pNode;
...
void Xfc::addLabelsRecTo(string path, vector<string> labels) throw(std::string)
{
    xmlNodePtr pNode;
    pNode = getNodeForPath(path);
    if (pNode == NULL)
...
void Xfc::removeLabelRecFrom(string path, string label) throw(string) {}

void Xfc::setDescriptionOf(std::string lPath, std::string lDescription) throw(std::string)
{
    xmlNodePtr lpNode;
    ...
    if (lS != "") {  // there is already a description
        xmlNodeSetContent(lpDescriptionNode,
                          (xmlChar *)lDescription.c_str());  // :bug: - encoding
    } else {
        xmlNodePtr lpNewNode;
        xmlNodePtr lpInsertBeforeNode;
...
XfcEntity Xfc::getEntityFromNode(xmlNodePtr lpNode) throw(std::string)
{
    XfcEntity lEnt(lpNode, this);
    return lEnt;
}

XfcEntity Xfc::getEntityFromPath(std::string path) throw(std::string)
{
...
void Xfc::setCDate(std::string lDiskName, std::string lCDate) throw(std::string)
{
    xmlNodePtr lpNode;
    ...
    if (!(lYear > 1900 && lYear < 2100 && c1 == '-' && c2 == '-' && lMonth >= 1 &&
          lMonth <= 12 && lDay >= 1 && lDay <= 31))
...
std::string Xfc::getCDate(std::string lDiskName, xmlNodePtr *lpCDateNode) const
    throw(std::string)
// ret "" if no cdate
{
    // :fixme: - check state
...
std::string Xfc::getCDate(xmlNodePtr lpNode, xmlNodePtr *lpCDateNode) const
    throw(std::string)
{
    std::string lS = "";
...
std::string Xfc::getChecksumOf(std::string lPath, std::string type) const
    throw(std::string)
{
    // :fixme: - check state
...
std::string Xfc::getChecksumOf(xmlNodePtr lpNode, std::string type) const
    throw(std::string)
{
    // :fixme: - check state
...
std::string Xfc::getVersionString() { return std::string(XFCAPP_VERSION); }

/**
 * helper function
 **/
EntityDiff Xfc::compareItems(string catalogName, XfcEntity &rEnt, string diskPath,
                             ...
                             volatile const bool *pAbortFlag)
{
    ...
    if (!isDirectory(diskPath)) {
        ...
        if (ostr.str() != details["size"]) {
            ...
            gLog << "catalog val=" << details["size"] << ", disk val=" << ostr.str()
                 << eol;
            return diff;
        }
        ...
        if (openFile) {
            ...
            if (hasSha256) {
                if (diskDetails[SHA256LABEL] != details[SHA256LABEL]) {
                    ...
                    diff.name =
                        diskPath.substr(pathPrefixLen + 1);  // including trailing '/'
 ...
 **/
int Xfc::verifyDirectory(string catalogPath, string diskPath, unsigned int pathPrefixLen,
                         ...
                         volatile const bool *pAbortFlag)
{
        ...
        if (isDirectory(diskPath + "/" + namesOnDisk[i])) {
            ...
            if (found) {
                verifyDirectory(catalogPath + "/" + namesOnDisk[i],
                                diskPath + "/" + namesOnDisk[i], pathPrefixLen,
                                pDifferences, pAbortFlag);
            } else {
                diff.type = eDiffOnlyOnDisk;
                diff.name = (diskPath + "/" + namesOnDisk[i]).substr(pathPrefixLen + 1);
                pDifferences->push_back(diff);
                gLog << xfcInfo << "Difference (only on disk) for: " << namesOnDisk[i]
                     << eol;
    ...
    for (unsigned int i = 0; i < namesInCatalog.size(); i++) {
        gLog << xfcDebug << __FUNCTION__
             << ": checking (catalog name): " << namesInCatalog[i] << eol;
        bool found = false;
