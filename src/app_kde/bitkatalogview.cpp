...
bitKatalogView::bitKatalogView(QWidget *parent) : QWidget(parent)
{
...
void bitKatalogView::print(QPainter *, int, int)
{
    // do the actual printing, here
    // p->drawText(etc..)
}

QString bitKatalogView::currentUrl() { return m_html->url().url(); }

void bitKatalogView::openUrl(QString url) { openUrl(KUrl(url)); }
...
void bitKatalogView::slotOnUrl(const QString &url) { emit signalChangeStatusbar(url); }
...
void bitKatalogView::setupListView()
{
    ...
    connect(mListView, SIGNAL(itemCollapsed(QTreeWidgetItem *)), this,
            SLOT(collapseItem(QTreeWidgetItem *)));
    connect(mListView, SIGNAL(customContextMenuRequested(const QPoint &)), this,
            SLOT(contextMenu(const QPoint &)));
}

void bitKatalogView::contextMenu(const QPoint &rPoint)
{
...
void bitKatalogView::details() throw()
{
    std::string completePath;
    ...
    try {
        ...
        pDetailsBox =
            new DetailsBox(mCatalog, completePath, &lEnt, mpCurrentItem, allLabels);
        pDetailsBox->exec();
        ...
        msgWarn("Cannot display info about this item! completePath=", completePath);
        msgWarn("  exception is: ", e);
        KMessageBox::error(this, "Cannot display informations about this item!");
    }
}

void bitKatalogView::addLabelRec() throw()
{
    std::string completePath;
    if (mCatalog == NULL) {
        KMessageBox::error(this, "No catalog!");
    ...
    if (completePath != "/") {
        ...
        if (QDialog::Accepted == pLabelsBox->exec()) {
            ...
            if (labelsToAdd.size() > 0) {
                vector<string> labelsToAddStdStr = vectWQStringToVectWstring(labelsToAdd);
                gkLog << xfcDebug << __FUNCTION__ << " adding new labels (rec.): " << eol;
...
void bitKatalogView::verifyDisk() throw()
{
    std::string completePath;
    std::string str;
    ...
    VerifyThread *pVerifyThread =
        new VerifyThread(mCatalog, completePath, qstr2cchar(dir), &differences);
        ...
        for (unsigned int i = 0; i < differences.size(); i++) {
            switch (differences[i].type) {
                case eDiffOnlyInCatalog:
                    // strip disk name
                    pResults->addText(
                        differences[i].name.substr(completePath.length() + 1), "!", "-");
                    break;
                ...
                case eDiffErrorOnDisk:
                    str1 = differences[i].name;
                    str2 = differences[i].name + "\n error reading details from disk: " +
                           differences[i].diskValue;
...
void bitKatalogView::renameDisk() throw()
{
...
void bitKatalogView::deleteDisk() throw()
{
    ...
    confirmDialog += " ?";
    if (KMessageBox::Yes == KMessageBox::warningYesNo(this, confirmDialog.c_str())) {
        try {
...
Xfc *bitKatalogView::getCatalog() { return mCatalog; }

void bitKatalogView::addFirstLevelElement(XfcEntity &rEnt)
{
 ...
 **/
int collectLabels(uint, const string &, Xfc &rCatalog, xmlNodePtr pNode, void *pParam)
{
...
void bitKatalogView::populateCatalogLabels()
{
    mCatalog->parseFileTree(collectLabels, this);
}

void bitKatalogView::addCatalogLabels(std::vector<std::string> newLabels)
{
 ...
 **/
void bitKatalogView::addCatalogLabel(std::string newLabel)
{
...
const std::vector<std::string> &bitKatalogView::getCatalogLabels() const
{
    return mCatalogLabels;
}

void bitKatalogView::expandItem(QTreeWidgetItem *pItem)
{
    static_cast<XmlEntityItem *>(pItem)->setOpened(true);
}

void bitKatalogView::collapseItem(QTreeWidgetItem *pItem)
{
