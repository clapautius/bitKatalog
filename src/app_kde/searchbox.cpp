 ...
 **/
static bool matchesName(const QFileInfo &rFsFileInfo, const string &rXmlPath,
                        const xmlNodePtr xmlNode, Xfc *pCat, KProgressDialog *)
{
    ...
    return fsIsFile == catalogIsFile &&
           rFsFileInfo.fileName().toStdString() == getLastComponentOfPath(rXmlPath);
}

static bool matchesSize(const QFileInfo &rFsFileInfo, const string &,
...
static bool matchesSha256sum(const QFileInfo &rFsFileInfo, const string &,
                             const xmlNodePtr xmlNode, Xfc *pCat,
                             KProgressDialog *pProgress)
{
    ...
    while (fin.good()) {
        ...
        if (fin.eof())
            lastTime = true;
        sha256UsingBufCallback(readBuf, fin.gcount(), firstTime, lastTime, &param, &value,
                               NULL);
...
SearchStruct::SearchStruct() { mpProgressDialog = NULL; }

SearchStruct::~SearchStruct() { clear(false); }

void SearchStruct::clear(bool freeMemory)
{
    if (freeMemory) {
        if (mpProgressDialog)
...
void SearchBox::editLabels()
{
    gkLog << xfcDebug << __FUNCTION__ << eol;
    string str;
    LabelsBox *pLabelsBox =
        new LabelsBox(vectWstringToVectWQString(mrAllLabels),
 ...
 * @retval -2 : error;
 **/
int findInTree(unsigned int depth __attribute__((unused)), const string &lPath,
               Xfc &lrXfc, xmlNodePtr lpNode, void *lpParam)
{
...
void SearchBox::findLocalFiles(bool exactly)
{
    if (mSearchStruct.mSearchResultsPaths.size() > 0) {
        ...
        KProgressDialog *pProgress = new KProgressDialog(
            this, "Searching ...", QString("Searching in folder ") + startPath);
        ...
        try {
            ...
            KMessageBox::error(this,
                               QString("Something bad happened. Error is: ") + e.c_str());
        } catch (...) {
            KMessageBox::error(this, QString("Something bad happened. Unknown error"));
        }
...
void SearchBox::matchByName(QDirIterator &rIt, vector<QFileInfo> &rResult,
...
void SearchBox::matchBySizeAndSha256(QDirIterator &rIt, vector<QFileInfo> &rResult,
                                     KProgressDialog *pProgressDlg)
{
    vector<MatchFuncType> functions;
 ...
 *   periodically.
 **/
void SearchBox::matchByRelation(vector<MatchFuncType> funcs, QDirIterator &rIt,
                                vector<QFileInfo> &rResult, KProgressDialog *pProgressDlg)
{
        ...
        for (uint i = 0; i < filesToSearch.size(); i++) {
            ...
            if (funcs[0](fInfo, filesToSearch[i], nodesToSearch[i], mpCatalog,
                ...
                for (uint j = 1; j < funcs.size(); j++)
                    if (!funcs[j](fInfo, filesToSearch[i], nodesToSearch[i], mpCatalog,
                                  pProgressDlg)) {
                        matchesAll = false;
                        break;
                    }
...
void SearchBox::findLocalFilesByName() { return findLocalFiles(false); }

void SearchBox::findLocalFilesExactly() { return findLocalFiles(true); }

void SearchBox::contextMenuEvent(QContextMenuEvent *event)
{
...
void SearchBox::selectAll() { mpSearchResults->selectAll(); }

void SearchBox::unselectAll() { mpSearchResults->clearSelection(); }
