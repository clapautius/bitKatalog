...
void msgWarn(std::string s, std::string s2, std::string s3)
{
    gkLog << xfcWarn << s;
    if (!s2.empty())
        gkLog << s2;
...
void msgInfo(std::string s, std::string s2, std::string s3)
{
    gkLog << xfcWarn << s;
    if (!s2.empty())
        gkLog << s2;
...
static void startUp(KCmdLineArgs *);

int main(int argc, char **argv)
{
    std::string otherText;
    KAboutData about(QByteArray("bitKatalog"), QByteArray("bitKatalog"),
                     ki18n("bitKatalog"), QByteArray(version),
                     ki18n("A KDE file & disk catalog that uses plain-text XML to store "
                           "catalog information."),
                     KAboutData::License_GPL_V2, ki18n("(C) 2017 Tudor Marian Pristavu"));

    about.addAuthor(ki18n("Tudor Marian Pristavu"), ki18n(""),
                    QByteArray("clapautiusAtGmailDotCom"));
    about.setProgramIconName("media-optical");

    KCmdLineArgs::init(argc, argv, &about);
    KCmdLineOptions options;
    options.add("verbose-level <argument>", ki18n("Verbose level (0-3)"), "0");
    options.add("xfclib-verbose-level <argument>",
                ki18n("Verbose level for xfclib (0-3)"), "0");
    ...
    QIcon windowIcon =
        KIconLoader::global()->loadIconSet("bitkatalog", KIconLoader::Small, 0, true);
    if (windowIcon.isNull()) {
        gkLog << xfcInfo << "Couldn't find bitKatalog icon" << eol;
        windowIcon = KIconLoader::global()->loadIconSet("media-optical",
...
std::vector<std::string> vectWQStringToVectWstring(std::vector<QString> v)
{
    vector<string> ret;
    for (uint i = 0; i < v.size(); i++) ret.push_back(qstr2cchar(v[i]));
    return ret;
}
...
{
    vector<QString> ret;
    for (uint i = 0; i < v.size(); i++) ret.push_back(str2qstr(v[i]));
    return ret;
}
 ...
 **/
const char *qstr2cchar(const QString &str) { return str.toUtf8().constData(); }

/**
 * Converts from QString to string, keeps utf-8 encoding.
 **/
string qstr2str(const QString &str)
{
 ...
 **/
QString str2qstr(const std::string &s) { return QString::fromUtf8(s.c_str()); }
